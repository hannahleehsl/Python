...
* CSE 160, Winter 2023
* Homework 4
* Description:This program finds the is to analyze the accuracy
    of the clustering of centroids.
* Collaboration:None
"""
...
def update_assignment(list_of_points, labels, centroids_dict):
    ...
    for point in list_of_points:
        ...
        if closest_centroid in closest_centroids:
            closest_centroids[closest_centroid].append(labels[index])
        else:
            closest_centroids[closest_centroid] = [labels[index]]
        index = index + 1
...
def majority_count(labels):
    ...
    maj_count = {}
    for label in labels:
        if label in maj_count:
            maj_count[label] += 1
        else:
            maj_count[label] = 1
    max_val = 0
    for label, count in maj_count.items():
        if count > max_val:
            max_val = count
    return max_val
...
def accuracy(list_of_points, labels, centroids_dict):
    ...
    Example:
        ...
        Output:
            0.6666666666666666
    """
    new_dict = update_assignment(list_of_points, labels, centroids_dict)
    total_sum = 0
    for centroid in new_dict.keys():
        sum_of_maj = majority_count(new_dict[centroid])
        total_sum = total_sum + sum_of_maj
    return total_sum / len(labels)
