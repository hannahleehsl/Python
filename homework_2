...
| 02/08 at 10:12 pm
Grading comment:
-0: Each substantial/complex section of your code should be preceded by a short comment that describes the purpose of that section of code.
...
def get_pixel_at(pixel_grid, i, j):
    ...
    if 0 <= i and i < len(pixel_grid) and 0 <= j and j < len(pixel_grid[0]):
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    for row_index in range(i - 1, i + 2):
        for col_index in range(j - 1, j + 2):
            pixel_sum = pixel_sum + \
                get_pixel_at(pixel_grid, row_index, col_index)
    return pixel_sum // 9
...
def blur(pixel_grid):
    ...
    for row_index in range(len(pixel_grid)):
        ...
        for col_index in range(len(pixel_grid[0])):
            temp_grid.append(average_of_surrounding(
                pixel_grid, row_index, col_index))
...
application = blur(input_grid)

# Step E: Write the result to both output files

write_image(out_image_filename, application)
write_grid(out_grid_filename, application)
